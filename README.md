...
(To avoid your credentials.json file being leaked, run `git update-index --assume-unchanged credentials.json` in your local repo so that changes to the file will be ignored by Git. See https://practicalgit.com/blog/make-git-ignore-local-changes-to-tracked-files.html.)
